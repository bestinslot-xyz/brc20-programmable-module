...
def generate_loop_inner_code(opcode, mode):
  ...
  if mode == Mode.min_stack:
    if category == InstructionCategory.nop:
      ...
      return get(OP_DUP1) + (stack_limit - 1) * (get(OP_DUP1) + get(opcode)) + get(OP_POP)
    elif category == InstructionCategory.push:
      return stack_limit * (push(opcode) + get(OP_POP))
    elif category == InstructionCategory.dup:
      n = opcode - OP_DUP1 + 1
      return (n - 1) * get(OP_DUP1) + (stack_limit - (n - 1)) * (get(opcode) + get(OP_POP)) + (n - 1) * get(OP_POP)
    ...
    if category == InstructionCategory.nullop:
      ...
      return stack_limit * get(OP_DUP1) + (stack_limit - 1) * get(opcode) + get(OP_POP)
    elif category == InstructionCategory.push:
      return stack_limit * push(opcode) + stack_limit * get(OP_POP)
    elif category == InstructionCategory.dup:
      n = opcode - OP_DUP1 + 1
      return (n - 1) * get(OP_DUP1) + (stack_limit - (n - 1)) * get(opcode) + stack_limit * get(OP_POP)
    else:
      print("Invalid opcode - LOOP")
      exit(1)
  else:
...
def generate_loop_v2(inner_code):
  counter = push_hex("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffb001")
  jumpdest_offset = len(counter) // 2
  return counter + get(OP_JUMPDEST) + inner_code + push_int(1) + get(OP_ADD) + get(OP_DUP1) + push_int(jumpdest_offset) + get(OP_JUMPI)
...
def get_all_bench_codes():
  ...
  for opcode in [OP_ADD, OP_MUL, OP_SUB, OP_SIGNEXTEND, OP_LT, OP_GT, OP_SLT, OP_SGT, OP_EQ, OP_AND, OP_OR, OP_XOR, OP_BYTE, OP_SHL, OP_SHR, OP_SAR]:
    params_list.append([opcode, Mode.min_stack])
    params_list.append([opcode, Mode.full_stack])
  for opcode in [OP_ADDRESS, OP_CALLER, OP_CALLVALUE, OP_CALLDATASIZE, OP_CODESIZE, OP_RETURNDATASIZE, OP_PC, OP_MSIZE, OP_GAS]:
    params_list.append([opcode, Mode.min_stack])
    params_list.append([opcode, Mode.full_stack])
...
for i in range(len(codes)):
  ...
  response = requests.post(url_mine, json=data_call, headers={'Content-Type': 'application/json'})
  js = response.json()
